...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-8 flex items-center gap-4">
        <a href="{{ route('admin.encyclopedia.index') }}" class="w-10 h-10 rounded-full bg-white border border-[#e8dbce] flex items-center justify-center text-gray-500 hover:text-[#a07954] hover:bg-[#FAF8F5] transition shadow-sm">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
        </a>
        <div>
            <p class="text-xs font-bold tracking-widest text-gray-400 uppercase mb-1">Perpustakaan Ensiklopedia Emosi</p>
            <h2 class="text-3xl font-serif font-bold text-[#1c1917]">Edit Entri Emosi</h2>
        </div>
    </div>
    ...
    <form action="{{ route('admin.encyclopedia.update', $encyclopedia) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="mb-10">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="feeling" value="{{ old('feeling', $encyclopedia->feeling) }}"
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="category"
                            ...
                            <option value="Positif"   {{ old('category', $encyclopedia->category) === 'Positif'   ? 'selected' : '' }}>Positif</option>
                            <option value="Sulit"     {{ old('category', $encyclopedia->category) === 'Sulit'     ? 'selected' : '' }}>Sulit</option>
                            <option value="Reflektif" {{ old('category', $encyclopedia->category) === 'Reflektif' ? 'selected' : '' }}>Reflektif</option>
                        </select>
        ...
        <div class="mb-10">
            ...
            <div x-data="{ preview: '{{ $encyclopedia->banner_url }}' }">
                <label class="block text-sm font-semibold text-gray-700 mb-2">Banner (Opsional, upload untuk mengganti)</label>
                <div class="w-full bg-white border-2 border-dashed border-[#d4c3b3] rounded-2xl flex flex-col items-center justify-center py-10 cursor-pointer hover:bg-gray-50 transition relative overflow-hidden"
                    ...
                    <template x-if="preview">
                        <img :src="preview" class="absolute inset-0 w-full h-full object-cover rounded-2xl opacity-60">
                    </template>
                    ...
                    <span class="text-sm font-medium text-[#1c1917] relative z-10" x-text="preview ? 'Ganti Banner' : 'Klik untuk upload banner'"></span>
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <textarea name="description" rows="2" required
                              ...
                              placeholder="Tuliskan ringkasan singkat emosi ini...">{{ old('description', $encyclopedia->description) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Pesan Validasi / Quote</label>
                    <input type="text" name="quote" value="{{ old('quote', $encyclopedia->quote) }}"
                           class="w-full bg-[#FAF8F5] border-0 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-[#a07954] outline-none shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] transition"
                ...
                <div>
                    ...
                    <textarea name="content" rows="6"
                              class="w-full bg-[#FAF8F5] border-0 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-[#a07954] outline-none shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] resize-none transition"
                              placeholder="Mulai menulis konten edukasi di sini...">{{ old('content', $encyclopedia->content) }}</textarea>
                </div>
            </div>
        </div>
...
function encyclopediaForm() {
    // Map existing tips to add an 'id' for x-for loop keying
