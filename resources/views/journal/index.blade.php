    ...
    >
        ...
        <div class="w-full bg-[#614d3c] rounded-[2rem] p-10 flex items-center justify-between shadow-lg mb-12 relative overflow-hidden">
            <div class="relative z-10 w-full text-center lg:text-left flex flex-col sm:flex-row items-center justify-center gap-3">
                ...
                <span class="text-4xl lg:text-5xl font-serif text-white/90 font-light">Jurnal dibuat</span>
            </div>
            ...
            <div class="lg:col-span-2 bg-white rounded-[2rem] p-8 shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-[#e8dbce]/30">
                <div class="flex justify-between items-center mb-8">
                    ...
                    <div class="flex items-center gap-3">
                        <button @click="prevMonth()"
                                class="w-8 h-8 rounded-full flex items-center justify-center border border-[#e8dbce] hover:bg-[#f7f4f0] transition text-gray-500">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                        </button>
                        <span class="text-sm font-medium text-gray-600 min-w-[120px] text-center" x-text="calMonthLabel"></span>
                ...
                <div class="grid grid-cols-7 gap-y-4 text-center mb-4">
                    <template x-for="d in ['Sen','Sel','Rab','Kam','Jum','Sab','Min']">
                        <div class="text-xs font-bold text-gray-400" x-text="d"></div>
                    </template>
                </div>
                    ...
                    <template x-for="day in daysInMonth" :key="day">
                        <div class="flex flex-col items-center gap-1">
                            <template x-if="isToday(day)">
                                <span class="w-7 h-7 rounded-full bg-[#614d3c] text-white text-sm font-bold flex items-center justify-center" x-text="day"></span>
                            </template>
                            <template x-if="!isToday(day)">
                                <span class="text-sm font-medium"
                                      :class="hasJournal(day) ? 'text-gray-800 font-bold' : 'text-gray-400'"
                                      x-text="day"></span>
                            </template>
                            <span x-show="hasJournal(day)" class="w-1.5 h-1.5 rounded-full bg-[#d4b996]"></span>
                        </div>
                    </template>
                </div>

                <div class="flex items-center justify-center gap-6 border-t border-gray-100 pt-6 mt-6">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="w-5 h-5 rounded-full bg-[#614d3c] flex items-center justify-center">
                            <span class="text-[8px] text-white font-bold" x-text="calToday"></span>
                        </span>
            ...
            <div class="flex flex-col gap-6">
                <div class="bg-[#e3dcd1] rounded-[2rem] p-8 text-center flex-1 flex flex-col justify-center shadow-inner relative overflow-hidden">
                    ...
                    <div class="relative z-10">
                        ...
                        @else
                            <p class="text-sm text-gray-500 mt-4 leading-relaxed">Belum ada jurnal.<br>Mulai tulis yang pertama!</p>
                        @endif
                    </div>
                </div>
                ...
                <form action="{{ route('journal.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        <input type="file" name="banner" accept="image/jpeg,image/png,image/webp"
                               x-ref="bannerInput" class="hidden" @change="handleBanner($event)">
                        <div @click="triggerBanner()"
                            ...
                            <div x-show="!bannerPreview" class="w-full h-full flex flex-col items-center justify-center gap-2 bg-[#faf8f5]">
                            ...
                            <div x-show="bannerPreview" class="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition flex items-center justify-center">
                    ...
                    <div class="mb-8">
                        <textarea name="description"
                                  ...
                                  rows="3" placeholder="Deskripsi singkat jurnal (opsional)">{{ old('description') }}</textarea>
                    </div>
